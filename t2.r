...
#  combining test and train into a single data frame
...
# setting seed to evaluate different runs of RF algo
set.seed(1234)
...
write.csv(submit.df, file = "RF_SUB_20190927_1.csv", row.names = FALSE)

# caret <- classification and regression training
library(caret)
# using dosnow to get past the single threaded memory limitations of R
library(doSNOW)

# setting to start at the same random point everytime as we are using multifolds
set.seed(2348)
# k = number fo folds, i.e. 10 folds 10 times
...
# doSow for multicore training,  we're going to be training a lot of trees.
# 6 child processes, using sock server <- simplest and best for single machine
...
set.seed(34324)
# method <- what do you want to train, in this case we train random forest "rf"
#tunelength try different values of tunegrid, here tunelength= 3, 3 different values
...
# problem is in family size, in 138 we predicted correctly for 83
